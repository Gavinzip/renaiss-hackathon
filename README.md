...
- Each identity submits three choices from three different teams. A one-choice ballot recorded before this rule changed remains locked as its original choice, but the voter may append exactly the two remaining eligible team choices; existing choices cannot be replaced or removed.
